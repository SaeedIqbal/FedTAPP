def trajectory_aware_aggregation(global_model, client_models, client_weights=None):
    """
    ...
    for client in client_models:
        ...
        for name, param in state.items():
            ...
            if param.shape != g_param.shape:
                aligned_state[name] = param  # Skip mismatched layers
                continue
    ...
    for key in global_state:
        avg_tensor = torch.mean(
            torch.stack([state[key] * w for state, w in zip(aligned_states, client_weights)], dim=0),
            dim=0
        )
